...
admin.initializeApp(functions.config().firebase);
	...
	function twitterHandle(chatPath, eventTitle, updateTime){
	...
	*/
	...
	exports.imageSender = functions.database.ref('/TestSuper/{subcate}/{matchname}/{matchid}/StadiumChat/{pushId}')
	.onWrite(event => {
		const node = event.data.val();
		console.log('Image Sender Call :');
		if(node.messageType=="image"){
			vision.detectLabels(storage.bucket("wazzon-bec6a.appspot.com").file(node.title)).then((results) => {
